  ...
  const { data: claimWaitData, isLoading: claimIsLoadingWaitData } =
    useWaitForTransaction({
      ...
      onError(error) {
        console.log(error);
        console.log('Could Not Register');
        alert("It seems you didn't attend this event, if you attended the event, pls contact MetaSquare support");
      },
    });
        ...
        <div>
            <div>
                ...
                <form onSubmit={handleSubmit}>
                    <button type="submit">
                        {regIsLoading || regIsLoadingWaitData ? 'Registering...' : 'Register'}
                    </button>
                </form>

                <form onSubmit={handleSubmit2}>
                    <button type="submit">
                        {claimIsLoading || claimIsLoadingWaitData ? 'Claiming Attendance NFT...' : 'Claim Attendance NFT'}
                    </button>
                </form>
            </div>
        </div>
